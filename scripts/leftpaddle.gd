...
var active = true
var up_input = "leftpaddle_up"
var down_input = "leftpaddle_down"
var velocity = 0.0
...
func _ready():
	if is_player_one:
		up_input = "leftpaddle_up"
		down_input = "leftpaddle_down"
	...
	if ball_count == 2:
		...
		if current_target_ball == 1:
			if !ai_target_coming1:
				switch_target = true
			elif ai_target_coming2 and ai_target_priority2 < ai_target_priority1 * 0.75:
				switch_target = true
		else:
